window.AppModule1 = (() => {
    ...
    function initApp() {
                    ...
                    const infoDialog = infoOverlay.querySelector('.info-dialog');
                    ...
                    async function captureClipAudioFallback(file, clipRange, progressBarEl, progressTextEl) {
                        ...
                        const clipDuration = Math.max(0.5, (clipRange.end || 0) - (clipRange.start || 0));
                        const targetSampleRate = 16000;

                        updateStatus("Extrayendo audio del clip sin reproducir el video...", true);
                        ...
                        const startSample = Math.max(0, Math.floor((clipRange.start || 0) * sourceSampleRate));
                        const endSample = Math.min(decodedBuffer.length, Math.ceil((clipRange.end || 0) * sourceSampleRate));
                        ...
                        return new File(
                            [wavBlob],
                            file.name.replace(/\.[^.]+$/, "") + "_clip_transcripcion.wav",
                            {
                                type: "audio/wav",
                    ...
                    async function createOptimizedAudioForTranscription(file, progressBarEl, progressTextEl, options = {}) {
                        ...
                        if (clipRange) {
                            return await captureClipAudioFallback(file, clipRange, progressBarEl, progressTextEl);
                        }
                        ...
                        try {
                            ...
                            const outputName = `transcribe_audio_${stamp}.webm`;
                            ...
                            const optimizedBlob = new Blob([data], { type: "audio/webm" });
                            return new File([optimizedBlob], file.name.replace(/\.[^.]+$/, "") + "_transcripcion.webm", {
                                type: "audio/webm",
                                lastModified: Date.now(),
                            });
                        } catch (error) {
                    ...
                    transcribeBtn.addEventListener("click", async () => {
                        ...
                        const wantsOptimizedAudio =
                            Boolean(activeClip) || shouldUseSegmentedTranscription;
                        ...
                        try {
                            ...
                            try {
                                ...
                                const shouldRetryWithOptimizedAudio =
                                    ...
                                    ((firstError?.status === 504) || String(firstError?.message || "").toLowerCase().includes("red"));

                                if (!shouldRetryWithOptimizedAudio) throw firstError;

                                updateStatus("Deepgram tardó demasiado; reintentando con audio optimizado...", true);
